...
class PhoneService:
    ...
    async def create_phone(self, uow: IUnitOfWork, phone_number_data: str) -> PhoneNumberSchemaResponse:
        async with uow:
            phone = await uow.phone.create_obj(phone_number_data)
            await uow.commit()
            return phone
        
    async def create_user_with_tel_number(self, uow: IUnitOfWork, **phone_number_id: str):
        async with uow:
    ...
    async def auth_phone(
        self, uow: IUnitOfWork, data_request_body: PhoneNumberSchema
    ):
        phone = await self.get_phone(uow=uow, phone_number=data_request_body.formatted_number)
        # далее стратегия!
        if phone is None:
        ...
        auth_context = Strategy.AuthContext(
            strategy=strategy
        )
